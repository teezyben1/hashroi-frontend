...
const createMiningPlan = async(e) => {
  ...
  return (
    <div className=' shadow-sm  h-full '>
        <div className={`${amountError} mb-3 bg-gray-200 w-[330px] rounded-sm`}><h1 className='text-red-600 text-sm font-nunito'>Amount should range from $50 - $500</h1></div>
    
        <form onSubmit={createMiningPlan} className='flex flex-col gap-2'>
            <div className='flex flex-col   '>
                
                <label className='text-lg font-nunito capitalize text-gray-900 font-semibold'>plan:</label>
                <input  type="text" className='font-nunito border-2 p-1 w-[200px] text-xl rounded-md capitalize border-gray-600' value={planName} disabled  />
            </div>
            <div className='flex flex-col   '>
