...
export const HalongPlan = () => {
    const [paymentMethod, setPaymentMethod] = useState('')
    const [amountInCrypto,setAmountInCrypto] = useState(0)
    const [amount,setAmount] = useState(1872)
    ...
    const [planName, setPlanName] = useState('Halong plan')
  ...
  const userId = userInfo.id
    ...
    const check = (e) => {
        ...
        e.preventDefault();
        setShowProfit('block')
        setProfit(10 / 100 * amount  )
        setTotal(10 / 100 * amount  + +amount)
        if(amount < 1872){
  ...
  return (
    <div className=' shadow-sm  h-full '>
        <div className={`${amountError} mb-3 bg-gray-200 w-[330px] rounded-sm font-nunito `}><h1 className='text-red-600 text-sm'>Amount should range from $1,872 - $9,800</h1></div>
